---
name: deactivate
description: Deactivate the Flowy ReVa overlay for this session (clears its routing obligation).
---

# Deactivate the ReVa overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate ReVa`

This forwards to the engine's DEACTIVATE path, which removes the `ReVa` entry from
