---
name: deactivate
description: Deactivate the Flowy superset overlay for this session (clears its routing obligation).
---

# Deactivate the superset overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate superset`

This forwards to the engine's DEACTIVATE path, which removes the `superset` entry from
