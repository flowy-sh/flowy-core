---
name: deactivate
description: Deactivate the Flowy open-design overlay for this session (clears its routing obligation).
---

# Deactivate the open-design overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate open-design`

This forwards to the engine's DEACTIVATE path, which removes the `open-design` entry from
