---
name: deactivate
description: Deactivate the Flowy pup overlay for this session (clears its routing obligation).
---

# Deactivate the pup overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate pup`

This forwards to the engine's DEACTIVATE path, which removes the `pup` entry from
