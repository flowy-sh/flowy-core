---
name: deactivate
description: Deactivate the Flowy claude-scholar overlay for this session (clears its routing obligation).
---

# Deactivate the claude-scholar overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate claude-scholar`

This forwards to the engine's DEACTIVATE path, which removes the `claude-scholar` entry from
