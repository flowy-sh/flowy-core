---
name: deactivate
description: Deactivate the Flowy woterclip overlay for this session (clears its routing obligation).
---

# Deactivate the woterclip overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate woterclip`

This forwards to the engine's DEACTIVATE path, which removes the `woterclip` entry from
