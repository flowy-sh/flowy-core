---
name: deactivate
description: Deactivate the Flowy ecc overlay for this session (clears its routing obligation).
---

# Deactivate the ecc overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate ecc`

This forwards to the engine's DEACTIVATE path, which removes the `ecc` entry from
