---
name: deactivate
description: Deactivate the Flowy superloopy overlay for this session (clears its routing obligation).
---

# Deactivate the superloopy overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate superloopy`

This forwards to the engine's DEACTIVATE path, which removes the `superloopy` entry from
