---
name: deactivate
description: Deactivate the Flowy deliberation overlay for this session (clears its routing obligation).
---

# Deactivate the deliberation overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate deliberation`

This forwards to the engine's DEACTIVATE path, which removes the `deliberation` entry from
