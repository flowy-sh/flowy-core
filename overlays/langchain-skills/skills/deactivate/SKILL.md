---
name: deactivate
description: Deactivate the Flowy langchain-skills overlay for this session (clears its routing obligation).
---

# Deactivate the langchain-skills overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate langchain-skills`

This forwards to the engine's DEACTIVATE path, which removes the `langchain-skills` entry from
