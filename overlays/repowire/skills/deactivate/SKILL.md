---
name: deactivate
description: Deactivate the Flowy repowire overlay for this session (clears its routing obligation).
---

# Deactivate the repowire overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate repowire`

This forwards to the engine's DEACTIVATE path, which removes the `repowire` entry from
