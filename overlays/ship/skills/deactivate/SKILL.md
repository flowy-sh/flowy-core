---
name: deactivate
description: Deactivate the Flowy ship overlay for this session (clears its routing obligation).
---

# Deactivate the ship overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate ship`

This forwards to the engine's DEACTIVATE path, which removes the `ship` entry from
