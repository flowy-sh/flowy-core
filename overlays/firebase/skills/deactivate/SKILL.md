---
name: deactivate
description: Deactivate the Flowy firebase overlay for this session (clears its routing obligation).
---

# Deactivate the firebase overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate firebase`

This forwards to the engine's DEACTIVATE path, which removes the `firebase` entry from
