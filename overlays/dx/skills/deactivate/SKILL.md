---
name: deactivate
description: Deactivate the Flowy dx overlay for this session (clears its routing obligation).
---

# Deactivate the dx overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate dx`

This forwards to the engine's DEACTIVATE path, which removes the `dx` entry from
