---
name: deactivate
description: Deactivate the Flowy cortex overlay for this session (clears its routing obligation).
---

# Deactivate the cortex overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate cortex`

This forwards to the engine's DEACTIVATE path, which removes the `cortex` entry from
