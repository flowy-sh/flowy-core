---
name: deactivate
description: Deactivate the Flowy openviking-memory overlay for this session (clears its routing obligation).
---

# Deactivate the openviking-memory overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate openviking-memory`

This forwards to the engine's DEACTIVATE path, which removes the `openviking-memory` entry from
