---
name: deactivate
description: Deactivate the Flowy ouroboros overlay for this session (clears its routing obligation).
---

# Deactivate the ouroboros overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate ouroboros`

This forwards to the engine's DEACTIVATE path, which removes the `ouroboros` entry from
