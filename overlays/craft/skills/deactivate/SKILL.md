---
name: deactivate
description: Deactivate the Flowy craft overlay for this session (clears its routing obligation).
---

# Deactivate the craft overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate craft`

This forwards to the engine's DEACTIVATE path, which removes the `craft` entry from
