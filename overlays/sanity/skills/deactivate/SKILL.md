---
name: deactivate
description: Deactivate the Flowy sanity overlay for this session (clears its routing obligation).
---

# Deactivate the sanity overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate sanity`

This forwards to the engine's DEACTIVATE path, which removes the `sanity` entry from
