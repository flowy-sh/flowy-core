---
name: deactivate
description: Deactivate the Flowy stark overlay for this session (clears its routing obligation).
---

# Deactivate the stark overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate stark`

This forwards to the engine's DEACTIVATE path, which removes the `stark` entry from
