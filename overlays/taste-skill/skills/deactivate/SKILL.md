---
name: deactivate
description: Deactivate the Flowy taste-skill overlay for this session (clears its routing obligation).
---

# Deactivate the taste-skill overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate taste-skill`

This forwards to the engine's DEACTIVATE path, which removes the `taste-skill` entry from
