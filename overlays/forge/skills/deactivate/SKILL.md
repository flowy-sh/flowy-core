---
name: deactivate
description: Deactivate the Flowy forge overlay for this session (clears its routing obligation).
---

# Deactivate the forge overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate forge`

This forwards to the engine's DEACTIVATE path, which removes the `forge` entry from
