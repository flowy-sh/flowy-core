---
name: deactivate
description: Deactivate the Flowy claude-blog overlay for this session (clears its routing obligation).
---

# Deactivate the claude-blog overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate claude-blog`

This forwards to the engine's DEACTIVATE path, which removes the `claude-blog` entry from
