---
name: deactivate
description: Deactivate the Flowy agentmemory overlay for this session (clears its routing obligation).
---

# Deactivate the agentmemory overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate agentmemory`

This forwards to the engine's DEACTIVATE path, which removes the `agentmemory` entry from
