---
name: deactivate
description: Deactivate the Flowy ck overlay for this session (clears its routing obligation).
---

# Deactivate the ck overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate ck`

This forwards to the engine's DEACTIVATE path, which removes the `ck` entry from
