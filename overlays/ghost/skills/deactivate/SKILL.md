---
name: deactivate
description: Deactivate the Flowy ghost overlay for this session (clears its routing obligation).
---

# Deactivate the ghost overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate ghost`

This forwards to the engine's DEACTIVATE path, which removes the `ghost` entry from
