---
name: deactivate
description: Deactivate the Flowy paperfit overlay for this session (clears its routing obligation).
---

# Deactivate the paperfit overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate paperfit`

This forwards to the engine's DEACTIVATE path, which removes the `paperfit` entry from
