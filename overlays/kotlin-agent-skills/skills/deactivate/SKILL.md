---
name: deactivate
description: Deactivate the Flowy kotlin-agent-skills overlay for this session (clears its routing obligation).
---

# Deactivate the kotlin-agent-skills overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate kotlin-agent-skills`

This forwards to the engine's DEACTIVATE path, which removes the `kotlin-agent-skills` entry from
