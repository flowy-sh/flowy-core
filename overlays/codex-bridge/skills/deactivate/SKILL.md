---
name: deactivate
description: Deactivate the Flowy codex-bridge overlay for this session (clears its routing obligation).
---

# Deactivate the codex-bridge overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate codex-bridge`

This forwards to the engine's DEACTIVATE path, which removes the `codex-bridge` entry from
