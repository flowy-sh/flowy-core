---
name: deactivate
description: Deactivate the Flowy nextjs overlay for this session (clears its routing obligation).
---

# Deactivate the nextjs overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate nextjs`

This forwards to the engine's DEACTIVATE path, which removes the `nextjs` entry from
