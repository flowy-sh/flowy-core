---
name: deactivate
description: Deactivate the Flowy claude-obsidian overlay for this session (clears its routing obligation).
---

# Deactivate the claude-obsidian overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate claude-obsidian`

This forwards to the engine's DEACTIVATE path, which removes the `claude-obsidian` entry from
