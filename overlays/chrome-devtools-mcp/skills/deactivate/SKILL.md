---
name: deactivate
description: Deactivate the Flowy chrome-devtools-mcp overlay for this session (clears its routing obligation).
---

# Deactivate the chrome-devtools-mcp overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate chrome-devtools-mcp`

This forwards to the engine's DEACTIVATE path, which removes the `chrome-devtools-mcp` entry from
