---
name: deactivate
description: Deactivate the Flowy agent-skills overlay for this session (clears its routing obligation).
---

# Deactivate the agent-skills overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate agent-skills`

This forwards to the engine's DEACTIVATE path, which removes the `agent-skills` entry from
