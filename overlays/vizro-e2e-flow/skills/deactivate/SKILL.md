---
name: deactivate
description: Deactivate the Flowy vizro-e2e-flow overlay for this session (clears its routing obligation).
---

# Deactivate the vizro-e2e-flow overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate vizro-e2e-flow`

This forwards to the engine's DEACTIVATE path, which removes the `vizro-e2e-flow` entry from
