---
name: deactivate
description: Deactivate the Flowy wigtn-plugins overlay for this session (clears its routing obligation).
---

# Deactivate the wigtn-plugins overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate wigtn-plugins`

This forwards to the engine's DEACTIVATE path, which removes the `wigtn-plugins` entry from
