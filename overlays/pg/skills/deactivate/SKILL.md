---
name: deactivate
description: Deactivate the Flowy pg overlay for this session (clears its routing obligation).
---

# Deactivate the pg overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate pg`

This forwards to the engine's DEACTIVATE path, which removes the `pg` entry from
