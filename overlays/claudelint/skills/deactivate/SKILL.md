---
name: deactivate
description: Deactivate the Flowy claudelint overlay for this session (clears its routing obligation).
---

# Deactivate the claudelint overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate claudelint`

This forwards to the engine's DEACTIVATE path, which removes the `claudelint` entry from
