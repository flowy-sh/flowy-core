---
name: deactivate
description: Deactivate the Flowy ponytail overlay for this session (clears its routing obligation).
---

# Deactivate the ponytail overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate ponytail`

This forwards to the engine's DEACTIVATE path, which removes the `ponytail` entry from
