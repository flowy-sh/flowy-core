---
name: deactivate
description: Deactivate the Flowy deadeye overlay for this session (clears its routing obligation).
---

# Deactivate the deadeye overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate deadeye`

This forwards to the engine's DEACTIVATE path, which removes the `deadeye` entry from
