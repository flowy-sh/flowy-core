---
name: deactivate
description: Deactivate the Flowy ralph-orchestrator overlay for this session (clears its routing obligation).
---

# Deactivate the ralph-orchestrator overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate ralph-orchestrator`

This forwards to the engine's DEACTIVATE path, which removes the `ralph-orchestrator` entry from
