---
name: deactivate
description: Deactivate the Flowy coder-eval overlay for this session (clears its routing obligation).
---

# Deactivate the coder-eval overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate coder-eval`

This forwards to the engine's DEACTIVATE path, which removes the `coder-eval` entry from
