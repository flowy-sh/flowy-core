---
name: deactivate
description: Deactivate the Flowy edgeone-makers-tools overlay for this session (clears its routing obligation).
---

# Deactivate the edgeone-makers-tools overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate edgeone-makers-tools`

This forwards to the engine's DEACTIVATE path, which removes the `edgeone-makers-tools` entry from
