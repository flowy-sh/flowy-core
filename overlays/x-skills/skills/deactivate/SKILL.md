---
name: deactivate
description: Deactivate the Flowy x-skills overlay for this session (clears its routing obligation).
---

# Deactivate the x-skills overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate x-skills`

This forwards to the engine's DEACTIVATE path, which removes the `x-skills` entry from
