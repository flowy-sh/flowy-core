...
> Routes 23 skills from `everyinc/compound-engineering-plugin` so the right one fires at the right phase.
>
> The plugin ships 33; the other 10 are not routed here.
...
USER MESSAGE
  ...
  ├─ does an open pull request need to be watched over time until it is merge ready rather than reviewed or resolved once?  → invoke compound-engineering:ce-babysit-pr   gate: a PR checkpoint report tracking merge readiness exists
  ├─ would committing to the wrong answer be expensive to unwind and only a cheap throwaway prototype can settle how it should work?  → invoke compound-engineering:ce-prototype   gate: a throwaway prototype exists that answers the open question
  ├─ is settled recently changed code ready to be simplified for clarity before review?  → invoke compound-engineering:ce-simplify-code   gate: the code is simplified with behaviour preserved
  ├─ does the task need a decisive verdict on adopting an external technology or reconciling other models opinions rather than new options or a document review?  → invoke compound-engineering:ce-pov   gate: a graded verdict with its reasoning exists
  └─ must the work or conversation continue in a future session with no access to this one?  → invoke compound-engineering:ce-handoff   gate: a handoff document exists for the next session to resume from
```
