---
name: deactivate
description: Deactivate the Flowy dotnet-artisan overlay for this session (clears its routing obligation).
---

# Deactivate the dotnet-artisan overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate dotnet-artisan`

This forwards to the engine's DEACTIVATE path, which removes the `dotnet-artisan` entry from
