---
name: deactivate
description: Deactivate the Flowy pinecone overlay for this session (clears its routing obligation).
---

# Deactivate the pinecone overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate pinecone`

This forwards to the engine's DEACTIVATE path, which removes the `pinecone` entry from
