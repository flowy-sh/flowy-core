---
name: deactivate
description: Deactivate the Flowy humanize overlay for this session (clears its routing obligation).
---

# Deactivate the humanize overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate humanize`

This forwards to the engine's DEACTIVATE path, which removes the `humanize` entry from
