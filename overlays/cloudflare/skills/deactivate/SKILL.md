---
name: deactivate
description: Deactivate the Flowy cloudflare overlay for this session (clears its routing obligation).
---

# Deactivate the cloudflare overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate cloudflare`

This forwards to the engine's DEACTIVATE path, which removes the `cloudflare` entry from
