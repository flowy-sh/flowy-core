---
name: deactivate
description: Deactivate the Flowy clone overlay for this session (clears its routing obligation).
---

# Deactivate the clone overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate clone`

This forwards to the engine's DEACTIVATE path, which removes the `clone` entry from
