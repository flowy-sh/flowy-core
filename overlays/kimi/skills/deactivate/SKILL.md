---
name: deactivate
description: Deactivate the Flowy kimi overlay for this session (clears its routing obligation).
---

# Deactivate the kimi overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate kimi`

This forwards to the engine's DEACTIVATE path, which removes the `kimi` entry from
