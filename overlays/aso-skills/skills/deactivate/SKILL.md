---
name: deactivate
description: Deactivate the Flowy aso-skills overlay for this session (clears its routing obligation).
---

# Deactivate the aso-skills overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate aso-skills`

This forwards to the engine's DEACTIVATE path, which removes the `aso-skills` entry from
