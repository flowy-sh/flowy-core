---
name: deactivate
description: Deactivate the Flowy recomby-geo overlay for this session (clears its routing obligation).
---

# Deactivate the recomby-geo overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate recomby-geo`

This forwards to the engine's DEACTIVATE path, which removes the `recomby-geo` entry from
