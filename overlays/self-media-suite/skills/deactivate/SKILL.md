---
name: deactivate
description: Deactivate the Flowy self-media-suite overlay for this session (clears its routing obligation).
---

# Deactivate the self-media-suite overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate self-media-suite`

This forwards to the engine's DEACTIVATE path, which removes the `self-media-suite` entry from
