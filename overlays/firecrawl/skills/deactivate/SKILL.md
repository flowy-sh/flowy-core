---
name: deactivate
description: Deactivate the Flowy firecrawl overlay for this session (clears its routing obligation).
---

# Deactivate the firecrawl overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate firecrawl`

This forwards to the engine's DEACTIVATE path, which removes the `firecrawl` entry from
