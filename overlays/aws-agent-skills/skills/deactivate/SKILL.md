---
name: deactivate
description: Deactivate the Flowy aws-agent-skills overlay for this session (clears its routing obligation).
---

# Deactivate the aws-agent-skills overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate aws-agent-skills`

This forwards to the engine's DEACTIVATE path, which removes the `aws-agent-skills` entry from
