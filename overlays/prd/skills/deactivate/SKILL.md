---
name: deactivate
description: Deactivate the Flowy prd overlay for this session (clears its routing obligation).
---

# Deactivate the prd overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate prd`

This forwards to the engine's DEACTIVATE path, which removes the `prd` entry from
