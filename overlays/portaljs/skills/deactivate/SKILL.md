---
name: deactivate
description: Deactivate the Flowy portaljs overlay for this session (clears its routing obligation).
---

# Deactivate the portaljs overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate portaljs`

This forwards to the engine's DEACTIVATE path, which removes the `portaljs` entry from
