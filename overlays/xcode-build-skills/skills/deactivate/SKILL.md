---
name: deactivate
description: Deactivate the Flowy xcode-build-skills overlay for this session (clears its routing obligation).
---

# Deactivate the xcode-build-skills overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate xcode-build-skills`

This forwards to the engine's DEACTIVATE path, which removes the `xcode-build-skills` entry from
