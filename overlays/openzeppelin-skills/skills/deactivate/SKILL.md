---
name: deactivate
description: Deactivate the Flowy openzeppelin-skills overlay for this session (clears its routing obligation).
---

# Deactivate the openzeppelin-skills overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate openzeppelin-skills`

This forwards to the engine's DEACTIVATE path, which removes the `openzeppelin-skills` entry from
