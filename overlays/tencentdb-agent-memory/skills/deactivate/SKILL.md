---
name: deactivate
description: Deactivate the Flowy tencentdb-agent-memory overlay for this session (clears its routing obligation).
---

# Deactivate the tencentdb-agent-memory overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate tencentdb-agent-memory`

This forwards to the engine's DEACTIVATE path, which removes the `tencentdb-agent-memory` entry from
