---
name: deactivate
description: Deactivate the Flowy academic-skills overlay for this session (clears its routing obligation).
---

# Deactivate the academic-skills overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate academic-skills`

This forwards to the engine's DEACTIVATE path, which removes the `academic-skills` entry from
