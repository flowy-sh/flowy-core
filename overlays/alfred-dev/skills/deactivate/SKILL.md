---
name: deactivate
description: Deactivate the Flowy alfred-dev overlay for this session (clears its routing obligation).
---

# Deactivate the alfred-dev overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate alfred-dev`

This forwards to the engine's DEACTIVATE path, which removes the `alfred-dev` entry from
