---
name: deactivate
description: Deactivate the Flowy superml overlay for this session (clears its routing obligation).
---

# Deactivate the superml overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate superml`

This forwards to the engine's DEACTIVATE path, which removes the `superml` entry from
