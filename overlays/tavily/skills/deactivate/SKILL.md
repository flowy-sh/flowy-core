---
name: deactivate
description: Deactivate the Flowy tavily overlay for this session (clears its routing obligation).
---

# Deactivate the tavily overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate tavily`

This forwards to the engine's DEACTIVATE path, which removes the `tavily` entry from
