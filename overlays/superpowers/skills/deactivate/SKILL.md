---
name: deactivate
description: Deactivate the Flowy superpowers overlay for this session (clears its routing obligation).
---

# Deactivate the superpowers overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate superpowers`

This forwards to the engine's DEACTIVATE path, which removes the `superpowers` entry from
