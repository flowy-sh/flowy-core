---
name: deactivate
description: Deactivate the Flowy resume-builder overlay for this session (clears its routing obligation).
---

# Deactivate the resume-builder overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate resume-builder`

This forwards to the engine's DEACTIVATE path, which removes the `resume-builder` entry from
