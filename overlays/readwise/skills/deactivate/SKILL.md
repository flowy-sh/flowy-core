---
name: deactivate
description: Deactivate the Flowy readwise overlay for this session (clears its routing obligation).
---

# Deactivate the readwise overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate readwise`

This forwards to the engine's DEACTIVATE path, which removes the `readwise` entry from
