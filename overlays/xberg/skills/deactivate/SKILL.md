---
name: deactivate
description: Deactivate the Flowy xberg overlay for this session (clears its routing obligation).
---

# Deactivate the xberg overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate xberg`

This forwards to the engine's DEACTIVATE path, which removes the `xberg` entry from
