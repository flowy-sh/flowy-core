---
name: deactivate
description: Deactivate the Flowy onchainos-skills overlay for this session (clears its routing obligation).
---

# Deactivate the onchainos-skills overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate onchainos-skills`

This forwards to the engine's DEACTIVATE path, which removes the `onchainos-skills` entry from
