---
name: deactivate
description: Deactivate the Flowy signum overlay for this session (clears its routing obligation).
---

# Deactivate the signum overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate signum`

This forwards to the engine's DEACTIVATE path, which removes the `signum` entry from
