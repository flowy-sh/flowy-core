---
name: deactivate
description: Deactivate the Flowy agent-starter overlay for this session (clears its routing obligation).
---

# Deactivate the agent-starter overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate agent-starter`

This forwards to the engine's DEACTIVATE path, which removes the `agent-starter` entry from
