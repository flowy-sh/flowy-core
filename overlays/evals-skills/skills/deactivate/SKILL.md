---
name: deactivate
description: Deactivate the Flowy evals-skills overlay for this session (clears its routing obligation).
---

# Deactivate the evals-skills overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate evals-skills`

This forwards to the engine's DEACTIVATE path, which removes the `evals-skills` entry from
