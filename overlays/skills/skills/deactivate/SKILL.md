---
name: deactivate
description: Deactivate the Flowy skills overlay for this session (clears its routing obligation).
---

# Deactivate the skills overlay

Invoke `flowy-core:_activator` with the argument:  `deactivate skills`

This forwards to the engine's DEACTIVATE path, which removes the `skills` entry from
